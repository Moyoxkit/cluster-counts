...
for redshift_index in range(41):
    ...
    for mass_index in range(36):
        ...
        bin_shift = int(
            np.round((np.log10(med) - midpoint) / (np.mean(np.diff(np.log10(c_ys)))))
        )
...
plt.plot(
    c_ys / 1e-5,
    rebinned_arr / len(sigs),
    label=r"Sim $M_{\rm 500c}$>$2\times10^{14}$ M$_{\odot}$",
)
plt.plot(
    c_ys / 1e-5,
    log_log_normal(c_ys, 0.73e-3, -5, np.sqrt(np.mean(sigs))),
    label="LN fit",
)
